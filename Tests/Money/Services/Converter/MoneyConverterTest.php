...
final class MoneyConverterTest extends TestCase
{
    ...
    public function testConvertIntToStringThrowExceptionIfValueIsLessZero(): void
    {
        ...
        $this->validator->expects(self::once())->method('isValidInt')->with(-1)->willReturn(false);
    ...
    {
        $this->validator->expects(self::once())->method('isValidInt')->with(0)->willReturn(true);
        $rtn = $this->converter->convertIntToString(0);
        self::assertSame('0,00', $rtn);
    }


    public function testConvertIntToStringReturnRightStringIfValueIsNotZero(): void
    {
        $this->validator->expects(self::once())->method('isValidInt')->with(123456)->willReturn(true);
        $rtn = $this->converter->convertIntToString(123456);
        self::assertSame('1.234,56', $rtn);
    }
    ...
    {
        $this->validator->expects(self::once())->method('isValidInt')->with(123456789)->willReturn(true);
        $rtn = $this->converter->convertIntToString(123456789, '.', ',', 3);
        self::assertSame('123.456,789', $rtn);
    }
}
